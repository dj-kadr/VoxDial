<?php
// blacklist.php — Управление стоп-листом с поддержкой группового удаления по примечаниям
...
// =========================================================================
// 2. УДАЛЕНИЕ ОДИНОЧНОГО НОМЕРА ИЛИ ГРУППЫ ИЗ СТОП-ЛИСТА 
// =========================================================================
if (isset($_GET['act']) && $_GET['act'] === 'delete') { 
    ...
    elseif (isset($_GET['desc'])) {
        ...
        if ($desc_to_delete === '[empty]') {
            $stmt = $pdo->prepare("DELETE FROM blacklist WHERE description IS NULL OR TRIM(description) = ''");
            $stmt->execute();
        } else {
            $stmt = $pdo->prepare("DELETE FROM blacklist WHERE description = ?");
            $stmt->execute([$desc_to_delete]);
        }
    }
...
$query_groups = "SELECT IF(description IS NULL OR TRIM(description) = '', '[empty]', description) as group_desc, 
                        COUNT(*) as count_phones 
                 FROM blacklist 
                 GROUP BY description 
                 ORDER BY count_phones DESC";
$notes_groups = $pdo->query($query_groups)->fetchAll(PDO::FETCH_ASSOC);
?>
<!DOCTYPE html>
<html lang="ru">
<head>
        ...
        <div class="col-md-10 p-4">
            ...
            <div class="row g-4">
                <div class="col-lg-4">
                    ...
                    <div class="card p-4 shadow-sm" style="background-color: #fffdec; border: 1px solid #f9f3c2;">
                        <h5 class="fw-bold mb-2 text-dark"><i class="fa-solid fa-layer-group text-warning me-2"></i>Очистка по группам</h5>
                        <p class="text-muted small mb-3">Здесь можно удалить из стоп-листа сразу все номера, объединенные одинаковым примечанием.</p>
                        
                        <div class="list-group" style="max-height: 280px; overflow-y: auto;">
                            <?php if (empty($notes_groups)): ?>
                                <div class="text-muted small py-2 text-center">Групп пока нет.</div>
                            <?php else: ?>
                                <?php foreach ($notes_groups as $group): 
                                    $is_empty = ($group['group_desc'] === '[empty]');
                                    $display_desc = $is_empty ? 'Без примечания (Пусто)' : $group['group_desc'];
                                    $url_desc = urlencode($group['group_desc']);
                                ?>
                                    <div class="list-group-item d-flex justify-content-between align-items-center p-2 group-item border-0 border-bottom">
                                        <div class="text-truncate me-2" style="max-width: 70%;">
                                            <span class="fw-bold text-dark small <?= $is_empty ? 'fst-italic text-muted' : '' ?>" title="<?= htmlspecialchars($display_desc) ?>">
                                                <?= htmlspecialchars($display_desc) ?>
                                            </span>
                                            <br>
                                            <small class="text-black-50" style="font-size: 0.75rem;">Номеров: <strong><?= $group['count_phones'] ?></strong></small>
                                        </div>
                                        <a href="blacklist.php?act=delete&desc=<?= $url_desc ?>" 
                                           class="btn btn-sm btn-danger px-2 py-1 small fw-bold" 
                                           style="font-size: 0.75rem;"
                                           onclick="return confirm('Вы уверены, что хотите полностью УДАЛИТЬ из стоп-листа все номера (<?= $group['count_phones'] ?> шт.) с примечанием: «<?= htmlspecialchars($display_desc) ?>»?')">
                                            <i class="fa-solid fa-trash-can me-1"></i> Стереть
                                        </a>
                                    </div>
                                <?php endforeach; ?>
                            <?php endif; ?>
...
<script>
    ...
    fileInput.addEventListener('change', function() { 
        if (this.files.length > 0) { 
            ...
            manualPhoneInput.removeAttribute('required'); 
